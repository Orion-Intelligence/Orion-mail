...
from orion.api.interactive.disposable_mailbox_manager.models.disposable_mailbox_param_model import DeleteDisposableMailboxRequest, GenerateDisposableMailboxRequest
...
async def generate_disposable_mailbox(data: GenerateDisposableMailboxRequest, current_user: db_user_model = Depends(get_current_user)):
    return await disposable_mailbox_manager.get_instance().generate_disposable_mailbox(
        current_user=current_user,
        pgp_key_id=data.pgp_key_id,
    )
...
async def delete_disposable_mailbox(disposable_id: str, data: DeleteDisposableMailboxRequest, current_user: db_user_model = Depends(get_current_user)):
    return await disposable_mailbox_manager.get_instance().delete_disposable_mailbox(
        current_user=current_user,
        disposable_id=disposable_id,
        keep_pgp=data.keep_pgp,
    )
